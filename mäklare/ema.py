...
class EMABot():
    """A trading bot that uses an Exponential Moving Average (EMA) crossover strategy.\n
    ...
    def find_options(self, price_data: dict | None = None):
        ...
        for t, df in price_data.items():
            ...
            if ema_short is None or ema_short.empty or ema_long is None or ema_long.empty:
...
def find_intersects(s0: pandas.Series, s1: pandas.Series):
    ...
    `s0` should be the shorter-period EMA series (e.g., ema9).
    `s1` should be the longer-period EMA series (e.g., ema21)."""
    ...
    # `pos` will be:
    #  1  if s0 > s1 (short EMA is above long EMA)
    # -1  if s0 < s1 (short EMA is below long EMA)
    #  0  if s0 == s1
...
if __name__ == "__main__":
    ...
    ema9 = ta.ema(df["PRICE"], 9)
    ema21 = ta.ema(df["PRICE"], 21)

    print(find_intersects(ema9, ema21))

    # Plot the price and EMAs for visual verification
    plt.plot(df.index, df['PRICE'], label='Price')
    plt.plot(df.index, ema9, label='9-period EMA')
    plt.plot(df.index, ema21, label='21-period EMA')
    plt.legend()
    plt.show()
