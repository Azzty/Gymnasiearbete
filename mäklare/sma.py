"""Använder korsniningar av två SMA kurvor för att ge köp/sälj signaler"""
...
class SMABot():
    ...
    `short_period` and `long_period` is the period for the short and long sma line respectively.
    ...
    def __init__(self, bot_name: str, tickers: list[str], risk: float = 0.02, short_period: int = 9, long_period: int = 21):
        ...
        # The two periods for the short and long SMA periods
    ...
    def find_options(self, price_data: dict | None = None):
        ...
        for t, df in price_data.items():
            # Calculate the short-period and long-period Exponential Moving Averages (SMA).
            sma_short = ta.sma(df['PRICE'], self.short_period)
            sma_long = ta.sma(df['PRICE'], self.long_period)

            if sma_short is None or sma_short.empty or sma_long is None or sma_long.empty:
                # print("PROBLEM WITH EMABOT: ema_short or ema_long is None")
                continue
            intersects = find_intersects(sma_short, sma_long)
            ...
            if intersects and [*intersects.keys()][-1] == target_timestamp:
                # Check the most recent crossover. 'over' means the short SMA crossed above the long SMA (a buy signal).
                # 'under' means the short SMA crossed below the long SMA (a sell signal).
...
def find_intersects(s0: pandas.Series, s1: pandas.Series):
    """Finds intersects between two pandas series using vectorized operations.
    Returns a dict of time and type of intersection of all intersections.
    `s0` should be the shorter-period SMA series (e.g., ema9).
    `s1` should be the longer-period SMA series (e.g., ema21)."""

    # Create a DataFrame to work with
    df = pandas.DataFrame({'s0': s0, 's1': s1}).dropna()

    # Determine the relationship between the two series at each point
    # `pos` will be:
    #  1  if s0 > s1 (short SMA is above long SMA)
    # -1  if s0 < s1 (short SMA is below long SMA)
    #  0  if s0 == s1
...
if __name__ == "__main__":
    ...
    # Calculate EMAs for the test data
    sma9 = ta.sma(df["PRICE"], 9)
    sma21 = ta.sma(df["PRICE"], 21)

    print(find_intersects(sma9, sma21))

    # Plot the price and EMAs for visual verification
    plt.plot(df.index, df['PRICE'], label='Price')
    plt.plot(df.index, sma9, label='9-period SMA')
    plt.plot(df.index, sma21, label='21-period SMA')
    plt.legend()
    plt.show()
